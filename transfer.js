function template_selector() {
    ...
    else if ($('#template_4').prop('checked') == true) {  // NEW TEMPLATE
...
function generateCV(template) {
    ...
    if (template == 'Template_3' || template == 'Template_4') { 
    ...
    document.querySelector('#dwnldimage').addEventListener('click', function () {
        let templateImage = $(`#${template}`).find('#target')[0];
        html2canvas(templateImage).then(function (canvas) {
    ...
    document.getElementById('pay-btn').addEventListener('click', function () {
        var options = {
            ...
            "amount": "200", // Amount in the smallest currency unit (50000 paise = 500 INR)
            "currency": "INR",
            "name": "Resume Builder",
            "description": "Payment for downloading the generated CV ",
            "handler": function (response) {
                // Handle success, send the response to the backend to verify
                console.log(response);
                // Call a backend API to verify the payment
            },
            "theme": {
                "color": "#F37254"
            }
    ...
    // **Education Section for Template 4**
    if (template == "Template_4") {
        $('.t4 .left-column .section:nth-child(3)').html(`
            <h2>Education</h2>
            <p><strong>${$('#degree').val()}</strong></p>
            <p>${$('#school').val()}</p>
            <p>${$('#edu_start').val()} - ${$('#edu_end').val()}</p>
        `);
    }
    
    // **Work Experience for Template 4**
    if (template == "Template_4") {
        let workHTML = "";
        $('#accordionWork .accordion-item').each(function () {
            let jobTitle = $(this).find('.job_title').val().trim();
            let company = $(this).find('.company_name').val().trim();
            let startDate = $(this).find('.work_start').val();
            let endDate = $(this).find('.end_date_toggle').prop('checked') ? 'Present' : $(this).find('.end_date').val();
            let description = $(this).find('.work_desc').val().trim();
        ...
        $('.t4 .right-column .section:nth-child(2)').html('<h2>Work Experience</h2>' + workHTML);
    }
    
    // **Skills for Template 4**
    if (template == "Template_4") {
        let skillsHTML = "";
